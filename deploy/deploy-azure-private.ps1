...
# copy build output into backend static folder if it exists
...
$backendBuildArgs = @()
if ($privatePipIndex) {
  $backendBuildArgs += "--build-arg"; $backendBuildArgs += "PIP_INDEX_URL=$privatePipIndex"
  $backendBuildArgs += "--build-arg"; $backendBuildArgs += "PIP_EXTRA_INDEX_URL=$privatePipIndex"
}
if ($privatePipToken) {
  $backendBuildArgs += "--build-arg"; $backendBuildArgs += "PRIVATE_PIP_TOKEN=$privatePipToken"
}
if ($privatePipUsername) {
  $backendBuildArgs += "--build-arg"; $backendBuildArgs += "PRIVATE_PIP_USERNAME=$privatePipUsername"
}
if ($pipTrustedHost) {
  $backendBuildArgs += "--build-arg"; $backendBuildArgs += "PIP_TRUSTED_HOST=$pipTrustedHost"
}
